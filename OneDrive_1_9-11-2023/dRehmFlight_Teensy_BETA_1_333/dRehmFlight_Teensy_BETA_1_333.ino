...
float maxRoll = 40;     //Max roll angle in degrees for angle mode (maximum ~70 degrees), deg/sec for rate mode 
float maxPitch = 25;    //Max pitch angle in degrees for angle mode (maximum ~70 degrees), deg/sec for rate mode
float maxYaw = 40;     //Max yaw rate in deg/sec 160
...
float Kp_pitch_angle = .25;   //Pitch P-gain - angle mode 0.2
float Ki_pitch_angle = .3;   //Pitch I-gain - angle mode 0.3
float Kd_pitch_angle = .08;  //Pitch D-gain - angle mode (has no effect on controlANGLE2) 0.05
float B_loop_pitch = 0.9;     //Pitch damping term for controlANGLE2(), lower is more damping (must be between 0 to 1)

float Kp_yaw = 0.2;          //Yaw P-gain 0.3
float Ki_yaw = .03;           //Yaw I-gain 0.05 (.01)
float Kd_yaw = 0.0001;       //Yaw D-gain (be careful when increasing too high, motors will begin to overheat!) 0.00015


//BELOW ARE PIDs that DO NOT APPPLY, as they are rate (Acro) mode

float Kp_pitch_rate = 0.0;   //Pitch P-gain - rate mode 0.15
float Ki_pitch_rate = 0.0;    //Pitch I-gain - rate mode 0.2
float Kd_pitch_rate = 0.0; //Pitch D-gain - rate mode (be careful when increasing too high, motors will begin to overheat!) 0.0002
...
void setup() {
  ...
  servo1.attach(servo1Pin, 500, 2400); //Pin, min PWM value, max PWM value
...
// VTOL MIXING:
  // Hover Mode
    //Tiltrotor Servos
      s1_command_scaled = (servo_vertical) + (1 * yaw_PID) - (1 * pitch_PID); //left servo
      s2_command_scaled = (1-servo_vertical) + (1 * yaw_PID) + (1 * pitch_PID); //right servo
    // Elevator Servos
      s3_command_scaled = 0; //left tail 
      s4_command_scaled = 0; //right tail(currently unused) 
    // Tiltrotor Motors
      s6_command_scaled = thro_des + (1 * roll_PID); //left motor
      s7_command_scaled = thro_des - (1 * roll_PID); //right motor
...
void getCommands() {
   ...
   */
    ...
    channel_2_pwm = getRadioPWM(4);
    channel_3_pwm = getRadioPWM(2);  
    channel_4_pwm = getRadioPWM(1);
    channel_5_pwm = getRadioPWM(5);
    channel_6_pwm = getRadioPWM(6);
    //PETER 4/19/2023 SWAPPED 2 and 4 to get the same control mapping as a quadcopter. Old lines below:
    //channel_1_pwm = getRadioPWM(3);
...
void throttleCut() {
  ...
  if (channel_5_pwm > 1500) {
    ...
    //PETER 4/19/2023 We are having issues with the failsafe throttlecut functionality, I have pinpointed the issue to the above lines. We are using servo PWM vars to control our BLDC motor ESCs, and the appropriate lines(s6 and s7) were commented out.
  }
}
